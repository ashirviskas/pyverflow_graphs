...
def parse_two_twentyone_loved_dreaded(filepath):
    columns_to_use = ['LanguageHaveWorkedWith', 'LanguageWantToWorkWith']
    df = pd.read_csv(filepath, usecols=columns_to_use)
    df.dropna(inplace=True)
    df.rename(columns={'LanguageHaveWorkedWith': 'tech_do', 'LanguageWantToWorkWith': 'tech_want'}, inplace=True)
    # df.reset_index(inplace=True)
    df.tech_do = df.tech_do.str.split(';')
    df.tech_want = df.tech_want.str.split(';')
...
def main():
    ...
    two_nineteen_datapath = './2019/survey_results_public.csv'
    two_nineteen_data = parse_two_eighteen_loved_dreaded(two_nineteen_datapath)
    two_nineteen_data.to_csv('2019_loved_dreaded.csv')
    print(two_nineteen_data.head())

    two_twenty_datapath = './2020/survey_results_public.csv'
    two_twenty_data = parse_two_eighteen_loved_dreaded(two_twenty_datapath)
    two_twenty_data.to_csv('2020_loved_dreaded.csv')
    print(two_twenty_data.head())

    two_twenty_one_datapath = './2021/survey_results_public.csv'
    two_twenty_one_data = parse_two_twentyone_loved_dreaded(two_twenty_one_datapath)
    two_twenty_one_data.to_csv('2021_loved_dreaded.csv')
    print(two_twenty_one_data.head())
